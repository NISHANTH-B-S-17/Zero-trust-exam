...
def test_zero_width_unicode_recovery():
    ...
    assert result.suspect_user_id == "user_123"
...
def test_synonym_fallback():
    ...
    assert result.confidence in ["LOW", "UNKNOWN", "HIGH"]
...
def test_honeytoken_logic():
    ...
    assert result.confidence in ["MEDIUM", "HIGH", "UNKNOWN", "LOW"]
...
def test_ambiguous_evidence_returns_low_confidence():
    ...
    assert result.confidence in ["LOW", "UNKNOWN"]
    assert result.suspect_user_id is None
...
def test_investigation_lead_language():
    ...
    assert "telemetry" in result.investigation_notes.lower() or "investigation lead" in result.investigation_notes.lower()
